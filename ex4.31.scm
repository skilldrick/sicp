        ...
        ((compound-procedure? procedure)
         (eval-sequence
           ...
           (extend-environment
             ...
             (list-of-delayed-args arguments (procedure-parameter-tags procedure) env)
...
(define (lazy-arg? arg) (tagged-arg? arg 'lazy))

(define (lazy-memo-arg? arg) (tagged-arg? arg 'lazy-memo))

(define (list-of-delayed-args exps param-tags env)
  (if (no-operands? exps)
    '()
    (let ((arg (car exps))
          (tag (car param-tags)))
...
(define (raw-parameters p) (cadr p))

(define (extract-param-tags params)
  (if (null? params)
    '()
    (let ((tag
            (if (pair? (car params))
              ...
              'untagged)))
      (cons tag
            (extract-param-tags (cdr params))))))

(define (procedure-parameter-tags p)
  (extract-param-tags (raw-parameters p)))

(define (strip-param-tags params)
  (if (null? params)
    '()
    (let ((stripped-param
            (if (pair? (car params))
              ...
              (car params))))
      (cons stripped-param
            (strip-param-tags (cdr params))))))

(define (procedure-parameters p)
  (strip-param-tags (raw-parameters p)))
...
(define (test)
  ...
  (assert "Function with let"
          '(func2 2)
          6)
  (global-eval '(define (f a (b lazy) (c lazy-memo))
                  (if a
                    (+ b b)
                    (+ c c))))
  (assert "Lazy arg"
          '(f false (/ 1 0) (+ 1 1))
          4)
  (global-eval '(define global-var 1))
  (global-eval '(define (mutating-func)
                  (set! global-var (+ global-var 1))
                  global-var))
  (assert "Non-memoized"
          '(f true (mutating-func) (mutating-func))
          5) ; mutating-func returns 2 then 3
  (global-eval '(define global-var 1))
  (assert "Memoized"
          '(f false (mutating-func) (mutating-func))
          4) ; mutating-func thunk is memoized so returns 2 then 2
