...
(define (make-lazy-list lst)
  (if (null? lst)
    '()
    (list 'cons (car lst) (make-lazy-list (cdr lst)))))
...
(define (add-lazy-lists-to-env env)
  ...
  (global-eval '(define (cdr z)
                  (z (lambda (p q) q))))
  (global-eval '(define (cadr z)
                  (car (cdr z))))
  (global-eval '(define (list-ref items n)
                  (if (= n 0)
                    (car items)
                    (list-ref (cdr items) (- n 1)))))
  (global-eval '(define (map proc items)
                  (if (null? items)
                    '()
                    (cons (proc (car items))
                          (map proc (cdr items))))))
  (global-eval '(define (scale-list items factor)
                  (map (lambda (x) (* x factor))
                       items)))
  (global-eval '(define (add-lists list1 list2)
                  (cond ((null? list1) list2)
                        ((null? list2) list1)
                        (else (cons (+ (car list1) (car list2))
                                    (add-lists (cdr list1) (cdr list2)))))))
  (global-eval '(define ones (cons 1 ones)))
  (global-eval '(define integers (cons 1 (add-lists ones integers))))
)
...
(define (setup-environment)
  (let ((initial-env
    ...
    (add-lazy-lists-to-env initial-env)
...
;          '(1 2 3))
  ...
  (assert "Lazy counting part 1"
          'count
          1)
  (assert "Lazy counting part 2"
          'w
          10)
  (assert "Lazy counting part 3"
          'count
          2)
  (assert "Cons and car"
          '(car (cons 1 2))
  ...
  (assert "Cons and cdr"
          '(cdr (cons 1 2))
          2)
  (assert "Car with list"
          '(car (cons 1 (cons 2 (cons 3 '()))))
          1)
  (assert "Cadr"
          '(cadr (cons 1 (cons 2 3)))
          2)
  (assert "List-ref with lazy lists"
          '(list-ref integers 17)
          18)
  (assert "Car of quoted list"
          '(car '(1 2 3))
          1)
